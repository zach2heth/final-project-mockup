...
if (Meteor.isServer) {
  describe('ProfileCollection', function testSuite() {
    ...
    const defineObject = { firstName, lastName, username, bio, interests, flairs, picture, title, github, facebook,
      instagram };
    ...
    it('#define, #isDefined, #removeIt, #dumpOne, #restoreOne', function test() {
      ...
      expect(doc.flairs[0]).to.equal(flairName);
    ...
    it('#define (illegal interest)', function test() {
      ...
      const defineObject2 = { firstName, lastName, username, bio, interests: illegalInterests, flairs, picture, title,
    ...
    it('#define (duplicate interests)', function test() {
      ...
      const defineObject3 = { firstName, lastName, username, bio, interests: duplicateInterests, flairs, picture, title,
        github, facebook, instagram };
      expect(function foo() { Profiles.define(defineObject3); }).to.throw(Error);
    });
    it('#define (illegal flair)', function test() {
      const illegalFlairs = ['foo'];
      const defineObject2 = { firstName, lastName, username, bio, interests, flairs: illegalFlairs, picture, title,
        github, facebook, instagram };
      expect(function foo() { Profiles.define(defineObject2); }).to.throw(Error);
    });

    it('#define (duplicate flairs)', function test() {
      const duplicateFlairs = [flairName, flairName];
      const defineObject3 = { firstName, lastName, username, bio, interests, flairs: duplicateFlairs, picture, title,
        github, facebook, instagram };
      expect(function foo() { Profiles.define(defineObject3); }).to.throw(Error);
    });
  });
